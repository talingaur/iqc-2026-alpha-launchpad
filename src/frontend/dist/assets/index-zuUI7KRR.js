...
const BookMarked = createLucideIcon("book-marked", __iconNode$9);
/**
 * @license lucide-react v0.511.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function parseDecayWindow(formula) {
...
function analyzeFormula(formula) {
  ...
  if (!f.includes("rank(") && !f.includes("ts_rank(")) {
    sharpe.push({
      ...
      reason: "rank() compresses extreme values and makes the signal distribution uniform across the universe, which directly improves Sharpe by reducing noise.",
      operator: "rank"
    });
  }
  if (!f.includes("group_neutralize(") && !f.includes("indneutralize(")) {
    sharpe.push({
      ...
      reason: "Without neutralization, sector-wide movements inflate apparent Sharpe. group_neutralize isolates stock-specific alpha from sector beta.",
  ...
  if (!f.includes("winsorize(") && !f.includes("pasteurize(")) {
    sharpe.push({
      ...
      reason: "Outlier alpha values distort the signal and inflate realized volatility. Winsorizing at 5% tails stabilizes Sharpe by controlling drawdown.",
      operator: "winsorize"
    });
  }
  if (!f.includes("ts_corr(") && !f.includes("ts_zscore(")) {
    sharpe.push({
      ...
      reason: "ts_zscore removes serial autocorrelation and adapts the signal to its own history, improving regime consistency and Sharpe stability.",
      operator: "ts_zscore"
    });
  }
  if (!f.includes("signed_power(") && !f.includes("log(")) {
    sharpe.push({
      ...
      reason: "Skewed signals produce lopsided return distributions. signed_power compresses the tail while preserving sign, giving a more symmetric payoff and better Sharpe.",
  ...
  if (f.includes("volume") && !f.includes("adv(")) {
    sharpe.push({
      ...
      reason: "Raw volume varies widely across stocks and time. Dividing by adv(20) makes the signal comparable across the universe, reducing bias and improving Sharpe.",
      operator: "adv"
    });
  }
  const fitness = [];
  ...
  if (!f.includes("decay_linear(")) {
    fitness.push({
      action: "Add decay_linear(signal, 10) to reduce turnover and improve fitness",
      reason: "High turnover is the #1 fitness killer. decay_linear with a 10-day window smooths daily signal changes, cutting unnecessary trades while preserving trend.",
    ...
    fitness.push({
      ...
      reason: "A decay window below 5 barely smooths the signal, resulting in high daily position churn that hurts fitness score and increases transaction costs.",
      operator: "decay_linear"
    });
  }
  if (!f.includes("pasteurize(") && !f.includes("truncate(") && !f.includes("winsorize(")) {
    fitness.push({
      action: "Add pasteurize() or truncate(signal, 0.01) to prevent extreme position weights",
      reason: "Extreme weights cause concentration risk. pasteurize() removes illiquid stocks; truncate() caps position sizes — both directly improve fitness checks.",
      operator: "pasteurize"
    });
  }
  if (!f.includes("scale(") && !f.includes("normalize(")) {
    fitness.push({
      action: "Wrap with scale(signal) or normalize(signal) to standardize position sizes",
      reason: "Brain expects book-neutral signals. scale() ensures the long and short legs are balanced, which is checked during fitness evaluation.",
      operator: "scale"
    });
  }
  let depth = 0;
  ...
  if (maxDepth > 3 && !f.includes("rank(")) {
    fitness.push({
      action: "Apply rank() at the outermost level to compress complex sub-expressions",
      reason: "Deeply nested expressions without a final rank() can produce unbounded values that fail Brain's submission checks and hurt fitness.",
      operator: "rank"
    });
  }
  const decay = [];
  if (decayWindow === null) {
    decay.push({
      ...
      reason: "A 10-day decay window balances signal freshness with turnover control. Without decay, even a good alpha will produce excessive daily churn.",
      operator: "decay_linear"
    });
  } else if (decayWindow < 5) {
    decay.push({
      action: `Decay window ${decayWindow} is very short — consider 7–10 to reduce excessive turnover`,
      reason: "Windows below 5 provide minimal smoothing. Increasing to 7–10 will significantly reduce turnover and improve both fitness and Sharpe consistency.",
      operator: "decay_linear"
    });
  } else if (decayWindow > 22) {
    decay.push({
      action: `Decay window ${decayWindow} is long — signal may become stale; consider 15–20`,
      reason: "Long decay windows over-smooth the signal, reducing its responsiveness to new information. This typically lowers alpha and can hurt Sharpe.",
      operator: "decay_linear"
    });
    ...
    decay.push({
      ...
      reason: "Short windows (5–10) work well for mean-reversion alphas. For trend-following formulas, a slightly longer window (13–18) can reduce noise-driven reversals.",
      operator: "decay_linear"
    });
  } else {
    decay.push({
      action: `Good decay range (${decayWindow}). If turnover is too high, try reducing to 7–10`,
      reason: "Mid-range windows (11–22) are typical for momentum alphas. If Brain reports high turnover, stepping down by 3–5 days usually helps.",
      operator: "decay_linear"
    });
  }
  ...
  const hasSubIndustry = f.includes("group_neutralize") && f.includes("subindustry");
  const hasIndNeutralize = f.includes("indneutralize(");
  const hasAnyNeutralization = hasSector || hasIndustry || hasSubIndustry || hasIndNeutralize || f.includes("market");
  if (!hasAnyNeutralization) {
    neutralization.push({
      action: "Add group_neutralize(signal, sector) as baseline neutralization",
      reason: "Without neutralization, broad market and sector moves dominate your alpha. group_neutralize(signal, sector) is the minimum baseline for competitive Brain submissions.",
      operator: "group_neutralize"
    });
  } else if (hasSector && !hasIndustry && !hasSubIndustry && !hasIndNeutralize) {
    neutralization.push({
      action: "Consider group_neutralize(signal, industry) instead of sector for tighter peer-group control",
      reason: "Industry-level neutralization removes more correlated noise than sector, reducing residual correlation between stocks and typically improving Sharpe.",
      operator: "group_neutralize"
    });
  } else if (hasIndustry && !hasSubIndustry) {
    neutralization.push({
      action: "Industry neutralization is strong. Try subindustry for maximum isolation",
      reason: "subindustry is the finest grouping available. If your alpha still shows peer-group correlation in backtests, subindustry neutralization can squeeze out additional Sharpe.",
      operator: "group_neutralize"
    });
  }
  if (hasSector && f.includes("rank(") && !hasSubIndustry && !hasIndNeutralize) {
    neutralization.push({
      action: "Consider adding indneutralize(signal, subindustry) on top of sector neutralization",
      reason: "Combining sector group_neutralize with indneutralize at the subindustry level gives a two-pass neutralization that maximizes residual alpha and improves Sharpe.",
      operator: "indneutralize"
    });
  }
  if (!f.includes("rank(") && hasAnyNeutralization) {
    neutralization.push({
      action: "Apply rank() at the outermost level to ensure market-neutral signal",
      reason: "rank() after neutralization ensures the final signal is strictly market-neutral (long book = short book), which is required for clean Brain submissions.",
      operator: "rank"
    });
  }
  return { sharpe, fitness, decay, neutralization };
}
function parseUserGoals(text) {
  const g = {
    targetSharpe: null,
    targetFitness: null,
    maxTurnover: null,
  ...
  if (lower.includes("sub universe") || lower.includes("sub-universe") || lower.includes("subuniverse") || lower.includes("sub sharpe")) {
    g.subUniverseSharpe = true;
  }
  if (lower.includes("neutral") || lower.includes("industry") || lower.includes("sector")) {
    g.improveNeutralization = true;
  }
  return g;
}
function buildImprovedAlpha(formula, analysis, goals) {
  const f = formula.toLowerCase();
  let result = formula.trim();
  const changes = [];
  const needsRank = !f.includes("rank(") && !f.includes("ts_rank(");
  const needsGroupNeutralize = !f.includes("group_neutralize(") && !f.includes("indneutralize(");
  const needsDecay = !f.includes("decay_linear(");
  const decayWindow = parseDecayWindow(formula);
  const needsWinsorize = !f.includes("winsorize(") && !f.includes("pasteurize(");
  ...
  const highSharpTarget = goals.targetSharpe !== null && goals.targetSharpe >= 1.25;
  const lowTurnoverGoal = goals.maxTurnover !== null && goals.maxTurnover <= 70;
  const highFitnessGoal = goals.targetFitness !== null && goals.targetFitness >= 1;
  ...
  if (needsWinsorize && (suggestedOps.has("winsorize") || highSharpTarget || highFitnessGoal)) {
    result = `winsorize(${result}, 0.05)`;
    changes.push("Added winsorize(0.05) — clips extreme values");
  }
  if (needsDecay && (suggestedOps.has("decay_linear") || lowTurnoverGoal || highFitnessGoal)) {
    const window2 = lowTurnoverGoal ? 15 : highFitnessGoal ? 12 : 10;
    result = `decay_linear(${result}, ${window2})`;
    changes.push(
      `Added decay_linear(${window2})${lowTurnoverGoal ? " — tuned for lower turnover" : ""}`
    );
  } else if (!needsDecay && decayWindow !== null && lowTurnoverGoal && decayWindow < 10) {
    const newWindow = 15;
  ...
  if (needsGroupNeutralize && (suggestedOps.has("group_neutralize") || goals.improveNeutralization || highSharpTarget)) {
    const level = goals.subUniverseSharpe ? "subindustry" : goals.improveNeutralization ? "industry" : "sector";
    result = `group_neutralize(${result}, ${level})`;
    changes.push(`Added group_neutralize(${level})`);
  }
  if (needsRank && (suggestedOps.has("rank") || highSharpTarget || highFitnessGoal)) {
    result = `rank(${result})`;
  ...
  if (needsScale && (suggestedOps.has("scale") || highFitnessGoal || lowTurnoverGoal)) {
    result = `scale(${result})`;
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-2", children: [
          s.operator && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "mt-0.5 flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(OpBadge, { name: s.operator }) }),
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              looksGood && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
        ...
        !looksGood && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-2 p-3", children: suggestions.map((s, i) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: static analysis list
          /* @__PURE__ */ jsxRuntimeExports.jsx(SuggestionItem, { s }, i)
        )) })
      ]
    }
  );
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0.5", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  className: "text-[10px]",
                  style: { color: "var(--text-secondary)" },
                  children: "Describe what Brain reported — system will fix it"
                }
              )
            ] })
          }
...
• sub universe sharpe is low`,
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-1.5", children: examples.map((ex) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              type: "button",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0.5", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "span",
                  {
                    className: "text-[10px]",
                    style: { color: "var(--text-secondary)" },
                    children: "Optimized for Brain submission targets"
                  }
                )
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] font-medium", children: copied ? "Copied!" : "Copy" })
                  ]
                }
              )
            ]
        ...
        changes.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-3 pb-3 flex flex-col gap-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-1", children: changes.map((change, i) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: static list
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#34d399", fontSize: "10px" }, children: "+" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
        ...
        changes.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-3 pb-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "span",
          {
            className: "text-[11px]",
            style: { color: "var(--text-secondary)" },
            children: "✓ No structural changes needed — formula looks complete."
          }
        ) })
      ]
    }
...
function AlphaAdvisor({ formula }) {
  const [goalsText, setGoalsText] = reactExports.useState("");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-3", "data-ocid": "alpha_advisor.panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-label", style: { flexShrink: 0 }, children: "Alpha Advisor" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "div",
        {
          className: "h-px flex-1",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-4", style: { alignItems: "flex-start" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { flex: "0 0 25%", minWidth: 0 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(GoalInput, { value: goalsText, onChange: setGoalsText }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { flex: "0 0 25%", minWidth: 0 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(ImprovedAlphaPanel, { improved, changes }) })
    ] })
  ] });
}
...
function AlphaLibrary({
  currentFormula,
  onLoad
}) {
  const [entries, setEntries] = reactExports.useState(() => loadFromStorage());
  const [alphaName, setAlphaName] = reactExports.useState("");
  const [category, setCategory] = reactExports.useState("Custom");
  const [nameError, setNameError] = reactExports.useState("");
  const [formulaError, setFormulaError] = reactExports.useState("");
  ...
  const handleCopy = reactExports.useCallback((entry) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { "data-ocid": "library.section", className: "flex flex-col gap-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        BookMarked,
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "span",
        {
          ...
          children: "persisted · localStorage"
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { className: "w-3 h-3", style: { color: "var(--muted-text)" } }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-label", children: "Save Current Formula" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                onFocus: (e) => {
                  e.currentTarget.style.borderColor = nameError ? "rgba(239,68,68,0.6)" : "rgba(59,130,246,0.4)";
                  e.currentTarget.style.boxShadow = "0 0 0 2px rgba(59,130,246,0.1)";
                },
                onBlur: (e) => {
                  e.currentTarget.style.borderColor = nameError ? "rgba(239,68,68,0.4)" : "var(--border-color)";
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "select",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "Cubic", children: "Cubic" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "Safety Gate", children: "Safety Gate" }),
          ...
          (nameError || formulaError) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0.5", children: [
            nameError && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px]", style: { color: "#f87171" }, children: nameError }),
            formulaError && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-[11px]", style: { color: "#f87171" }, children: formulaError })
          ] })
        ]
      }
    ),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
                ...
                children: "Save your first formula above."
              }
            )
          ] })
        ]
      }
    ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-2", children: entries.map((entry, index2) => {
      const catStyle = CATEGORY_STYLES[entry.category];
      const cardOcid = index2 < 3 ? `library.item.${index2 + 1}` : void 0;
      const copyOcid = index2 < 3 ? `library.copy_button.${index2 + 1}` : void 0;
      const loadOcid = index2 < 3 ? `library.load_button.${index2 + 1}` : void 0;
      const deleteOcid = index2 < 3 ? `library.delete_button.${index2 + 1}` : void 0;
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-3", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                ...
                title: entry.formula,
                children: entry.formula.length > 80 ? `${entry.formula.slice(0, 80)}…` : entry.formula
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDate(entry.savedAt) })
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-1.5", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  style: {
                    backgroundColor: isCopied ? "rgba(16,185,129,0.12)" : "rgba(255,255,255,0.04)",
                  ...
                  onMouseEnter: (e) => {
                    if (!isCopied) {
                      ...
                      e.currentTarget.style.borderColor = "var(--border-color-bright)";
                  ...
                  children: [
                    isCopied ? /* @__PURE__ */ jsxRuntimeExports.jsx(CheckCheck, { className: "w-3 h-3" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { className: "w-3 h-3" }),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  onMouseEnter: (e) => {
                    ...
                    e.currentTarget.style.borderColor = "rgba(59,130,246,0.4)";
                    e.currentTarget.style.boxShadow = "0 0 8px rgba(59,130,246,0.12)";
                  },
                  onMouseLeave: (e) => {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  className: "flex items-center gap-1.5 px-2.5 py-1.5 rounded text-[11px] font-medium transition-all duration-150",
                  style: {
                    backgroundColor: isConfirmingDelete ? "rgba(239,68,68,0.12)" : "rgba(255,255,255,0.04)",
                    border: `1px solid ${isConfirmingDelete ? "rgba(239,68,68,0.4)" : "var(--border-color)"}`,
                    color: isConfirmingDelete ? "#f87171" : "var(--muted-text)",
                    cursor: "pointer"
                  },
                  onMouseEnter: (e) => {
                  ...
                  title: isConfirmingDelete ? "Click again to confirm deletion" : "Delete this alpha",
                  children: [
...
function highlightFormula(code) {
  ...
  const tokens = [];
  const addTokens = (regex, color) => {
    let match = regex.exec(escaped);
    while (match !== null) {
  ...
  addTokens(new RegExp("(?<!ts_)rank", "g"), "#22c55e");
  addTokens(
...
const FormulaEditor = reactExports.forwardRef(
  ({ value, onChange, onCursorPositionChange }, ref) => {
    const textareaRef = reactExports.useRef(null);
    const insertAtCursor = reactExports.useCallback(
      (text) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between px-0.5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-label", children: "Formula Editor" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            ...
            style: {
              ...
              width: "320px",
              height: "140px",
              background: "radial-gradient(ellipse at 30% 50%, rgba(59,130,246,0.07) 0%, transparent 70%)",
              pointerEvents: "none"
            }
          }
        ),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "h1",
              {
                ...
                style: {
                  background: "linear-gradient(105deg, #93c5fd 0%, #38bdf8 50%, #67e8f9 100%)",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
                ...
                children: "WorldQuant Brain Formula Studio"
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2.5 relative z-10", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              className: "flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs font-mono font-medium",
              style: {
                backgroundColor: charCount > 0 ? "rgba(59,130,246,0.08)" : "rgba(255,255,255,0.04)",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "span",
                  {
                    style: {
                      ...
                      boxShadow: charCount > 0 ? "0 0 5px rgba(59,130,246,0.9)" : "none",
                      transition: "all 0.2s ease",
                      flexShrink: 0
                    }
                  }
...
function OperatorSidebar({ onInsert }) {
  const [search, setSearch] = reactExports.useState("");
  const filtered = reactExports.useMemo(() => {
    const q = search.toLowerCase().trim();
    if (!q) return OPERATORS;
    return OPERATORS.filter(
      (op) => op.name.toLowerCase().includes(q) || op.def.toLowerCase().includes(q)
    );
  }, [search]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "aside",
    {
      ...
      children: [
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  ...
                  children: "Click to insert at cursor"
                }
              )
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto py-1.5", style: { minHeight: 0 }, children: filtered.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
        ...
        ) : /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "flex flex-col px-1.5", children: filtered.map((op, index2) => {
          const ocid = index2 < 5 ? `sidebar.item.${index2 + 1}` : void 0;
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[11px]", style: { color: "var(--muted-text)" }, children: "inserts operator at cursor" })
            ]
          }
        )
      ]
...
const SAMPLE_FORMULA = "rank(ts_rank(close / ts_delay(close, 5), 20) - decay_linear(returns, 10)) * group_neutralize(volume / adv(20), sector)";
function App() {
  const [formula, setFormula] = reactExports.useState(SAMPLE_FORMULA);
  const setCursorPos = reactExports.useState(0)[1];
  const editorRef = reactExports.useRef(null);
  const handleInsert = reactExports.useCallback((text) => {
    var _a2;
    (_a2 = editorRef.current) == null ? void 0 : _a2.insertAtCursor(text);
  }, []);
  const handleLoad = reactExports.useCallback((f) => {
    setFormula(f);
    setTimeout(() => {
      var _a2;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Header, { charCount: formula.length }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-1 overflow-hidden", style: { minHeight: 0 }, children: [
            ...
            "main",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-5 p-5", children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("section", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(AlphaAdvisor, { formula }) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      ...
                      children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-label", style: { flexShrink: 0 }, children: "Alpha Library" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "div",
                          {
                            className: "h-px flex-1",
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "pb-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(AlphaLibrary, { currentFormula: formula, onLoad: handleLoad }) })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "footer",
                  {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              ...
              children: /* @__PURE__ */ jsxRuntimeExports.jsx(OperatorSidebar, { onInsert: handleInsert })
            }
          )
        ] })
      ]
